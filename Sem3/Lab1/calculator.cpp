...
calculator::calculator(const std::string& sin,std::ostream& out) :iteration(0),out_stream(out) {
    _sin = new std::stringstream;
...
double calculator::get_value() {
    ...
    while (std::find(act_symbols, act_symbols + 6, (*_sin).get()) == act_symbols + 6 && !(*_sin).eof())
    {
        (*_sin).unget();
        char symb = (*_sin).get();
        if (!isalpha(symb) && symb != -1) buffer_value += symb;
        else
        {
            (*_sin).unget();
            break;
...
void calculator::calculate() {
    ...
    out_stream << res << " = " << result << "\r\n";
    (*variables).emplace(res,result);
}
...
void calculator::set_variables() {
    while (!(*_sin).eof())
    {
        if ((*_sin).get() != ' ') {
            ...
            if (name.empty()) break;
            (*_sin).get();
            (*variables).emplace(name,get_value());
        }
    }
}
...
void calculator::build_tree() {
    ...
    while (!(*_sin).eof()) {
        ...
        if (isalpha(symb) || isdigit(symb)) {
            ...
            case MUL: {
                binary_tree<var>::leaf _leaf = tree.create_Node({ cur_act, 0 }, nullptr);
                if (cur->value.act == VAR || cur->value.act==DIV || cur->value.act==MUL) {
                    _leaf->next_l = cur;
                    cur->prev = _leaf;
                    cur = _leaf;
                }
